...
author: "Tin Ho, tin@berkeley.edu"
...
tmp1 = ggplot(CS_data, aes( x=CS_rate_100, y=GDP_2006) ) + geom_line() + geom_point() +
  theme_minimal(base_size=14)
#tmp1

tmp2 = ggplot(CS_data, aes( y=CS_rate_100, x=GDP_2006) ) + geom_line() + geom_point() +
  theme_minimal(base_size=14)
#tmp2

p1 <- ggplot(CS_data, aes( y=CS_rate_100, x=GDP_2006) ) + geom_point() 
p1
...
#View(CS_data)
CS_data_log <- CS_data %>% mutate(log_CS=log(CS_rate_100,exp(1)), log_GDP=log(GDP_2006))
#View(CS_data_log)
...
p3 <- ggplot(CS_data_log, aes( y=log_CS, x=log_GDP) ) + geom_point() 
p3
# YOUR CODE HERE
```
...
Not quite, it was linear up to a point, then plateau out. 
Was this called a threshold level relationship?
...
### lab video 2 [18:28]
aes(col=Income_Group)
# YOUR CODE HERE
###p3 = ggplot(CS_data_log, aes( y=log_CS, x=log_GDP) ) + geom_point() 
###p6 = ggplot(CS_data_log, aes( y=log_CS, x=log_GDP) ) + geom_point() 

#p6 = p4 + aes(col=Income_Group)
#p6 = p3 + aes(col=Income_Group) + geom_smooth()

p6 <- ggplot(CS_data_log, aes( y=log_CS, x=log_GDP) ) + geom_point(aes(col=Income_Group)) + geom_smooth() 
p6
...
_Type your answer here, replacing this text._
It holds for the low income group.
...
# p8
p8 <- ggplot(CS_data_sub, aes( y=log_CS, x=log_GDP) ) + geom_point(aes(col=Income_Group)) + geom_smooth() 
p8
# YOUR CODE HERE
```
...
lin_mod =  lm(log_CS ~ log_GDP, data=CS_data_sub)
##p9 = ggplot(lin_mod, aes( y=log_CS, x=log_GDP) )
#XXtmp9 = tidy(lin_mod)
#XXtmp9
#XXp9 = ggplot(tmp9, aes( y=log_CS, x=log_GDP))
p9 <- lin_mod
p9
# YOUR CODE HERE
```
...
_Type your answer here, replacing this text._
slope is 0.8183, which means that, at the natural log scale, an increase of 1 unit in the GDP corresponds to an increase of 0.8 in the log(%Cesarean) rate.  exact number would need to raise with exponent e.  eg exp(y)
...
_Type your answer here, replacing this text._
y = mx + b
m = 0.8193     ### m aka b in this class
x = log(2000)
b = -3.9405
y = m*x + b
y is 2.286919,
so CS rate is exp(y), which becomes 9.84%
...
_Type your answer here, replacing this text._
GDP of 50,000 would be in the upper income level, which the model was not linear.
From what we have seen, it has plateaus out.
If we use the linear fit, the line would be much higher, way over-estimating CS rate.

as eg, let's plug in the values.
xx = log(50000)
y = m*xx + b 
y become 4.91, exp(y) becomes 137.57%, which is impossible :D

### y=exp(m*xx+b) for CS in %
