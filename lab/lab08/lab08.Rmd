...
deeks1 <- read.csv("data/deeks_ex1.csv") %>% mutate(
  AgeGrp = case_when( Age==0 ~ "Grp0", Age==1 ~ "Grp1")
)
#View(deeks1)
p1 <- 'YOUR GGPLOT CODE HERE'
p1 <- ggplot(deeks1, aes(y=ave_cd4, x=AgeGrp)) + geom_jitter(width=0.1) + geom_boxplot(alpha=0.4)
p1
```
...
## I will be using 2-sample independent t-test


pvalue_deeks <- 'REPLACE WITH P-VALUE ROUNDED TO 2 DECIMAL PLACES'

## manual method, see wk11 p20


deeksMeanByGrp <- deeks1 %>% group_by(Age) %>%  summarise( mean = mean(ave_cd4), 
                                                           sd   = sd(ave_cd4),
                                                           length = length(ave_cd4)
                                                         )
deeksMeanByGrp

mean_grp0 <- deeksMeanByGrp$mean[1]
mean_grp1 <- deeksMeanByGrp$mean[2]
sd_grp0   <- deeksMeanByGrp$sd[1]
sd_grp1   <- deeksMeanByGrp$mean[2]
n_grp0    <- deeks1 %>% filter(Age==0) %>% nrow()  ## 153 (not 155)
n_grp1    <- deeks1 %>% filter(Age==1) %>% nrow()  ## 144   ## n_total=297

#print( c( n_grp0, n_grp1 ) )

#print( c( mean_grp0, mean_grp1, sd_grp0^2, n_grp0,  sd_grp1^2, n_grp1 ) )

meanDiff <-  mean_grp0 - mean_grp1   # reversing this just reverse sign of value for t_score
SE       <-  sqrt(   sd_grp0^2/n_grp0  +  sd_grp1^2/n_grp1    )    ## mg0 - mg1: 24.37888  --> t=-0.9943196

#xx SE       <-  sqrt(  (sd_grp0^2 + sd_grp1^2) / (n_grp0+n_grp1)    )  ## 17.11732  --> t=-1.416133


t_score  <-  meanDiff / SE

#t_score <- ( mean_grp0 - mean_grp1 ) / sqrt( ((sd_grp0^2/n_grp0) + (sd_grp1^2/n_grp1))  )
#t_score <- ( mean_grp1 - mean_grp0 ) / sqrt(   sd_grp0^2/n_grp0  +  sd_grp1^2/n_grp1    )



t_score  # -0.9943196, does not match result from t.test() :-\

# asking on piazza: https://piazza.com/class/kyf23hxwr4r52s?cid=334
# lecture didn't cover how to get pvalue using the t-score.... 
# is it pnorm?  or pt()?   we have never seen a "t-distribution" plot.  what does it look like?  check textbook?
# pt(-5.0723, lower.tail=T, df=198) 

# formula from class didn't specify abs(), t-test is symetric? 

manual_pvalue_deeks <- pt(q=t_score, df=n_grp1+n_grp1-2, lower.tail=T)  # compare to t.test() result
##xx manual_pvalue_deeks <- pt(q=t_score, df=n_grp1+n_grp1-2, lower.tail=T)  *2
##// manual_pvalue_deeks <- pt(q=-1.2563, df=n_grp1-2, lower.tail=F)*2
#manual_pvalue_deeks


pvalue_deeks_R <-   t.test(ave_cd4 ~ AgeGrp, alternative="two.sided" , data=deeks1)  # p.value=0.21
# t = -1.2563, df = 286.52, p-value = 0.21
pvalue_deeks_R

## Lab08 video @ 10:45 only used t.test() ... 
video_pvalue_deeks <- t.test(deeks1 %>% filter(Age==0) %>% pull( ave_cd4 ),
                       deeks1 %>% filter(Age==1) %>% pull( ave_cd4 ),
                       alternative = "two.sided" )
#video_pvalue_deeks
# t = -1.2563, df = 286.52, p-value = 0.21  ## same as what I got from t.test() above
...
deeks2 <- read.csv("data/deeks_ex2.csv") 
deeks2 %>% head(6)

deeks_wide <- NULL # YOUR CODE HERE

# lab08 wk11 video @ 14:41
#deeks_wide <- deeks2 %>% spread(medvl, cd4 )

# YOUR CODE HERE
deeks_wide <- deeks2 %>% pivot_wider( id_cols=id, names_from=medvl, values_from=cd4 )
deeks_wide  %>% head(6)
...
deeks_wide <- rename( deeks_wide, high="0", low="1") %>% mutate( diff=high-low)
deeks_wide %>% head(6)
...
p5 <- ggplot( data=deeks_wide, aes(x=diff)) + 
  geom_histogram(color="white", fill="lightgray") + 
  geom_vline(aes(xintercept=mean(diff),color="red")) + 
...
## not sure why the spagetti plots were added in the lab video, hopefully my lab08.Rmd question list is up-to-date ?
p5_i <- ggplot(deeks2, aes( y=cd4, x=medvl)) + geom_point( aes(color=id)) + 
  geom_line( aes( group=id) )
p5_i

p5_ii <- ggplot(deeks2, aes( y=cd4, x=medvl)) + geom_point( aes(color=id)) + 
  geom_line( aes( group=id) ) + facet_wrap( ~id )
p5_ii
...
paired_t <- t.test( deeks_wide$high , deeks_wide$low, 
                    alternative='two.sided', paired=T )
paired_t
p6 <- round(paired_t$p.value, 4)
p6
...
```
_The estimated mean difference is 48.28 (unit) and the 95% CI rane is (16.59, 79.97) _

\newpage

### Section II: Coin Flip Game.

// no lab video for this section.
...
#### right hand score: 22
#### left  hand score: 22   ### well, that confirms i am neither left nor right handed :P
...
#my_key <- "1f04hHZA7Vn36Xu62iWStBPnX6oSAAE7LGOStN9rwjmA"
```
...
# I got permission denied... so probably can't proceed further.
``` 
...
#our_sheet_diff <- our_sheet %>% mutate( diff = )
