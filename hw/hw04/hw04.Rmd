...
//sn50// could not get grader to work at home, moved to use datahub 
...
#### no it is not,  nowhere did i see rbinom !!
...
| Birth Defect  |      BD      |  noBD |
|=====----------|--------------|-------|
| prob          | 1/33=30.30%  | 100-30.30= 69.70%    |

_sample space is discrete, cuz it is counting number of birth defect_    #correct
...
?integer
birth_id = 1:200
bd = c( rep( integer(1),ceiling(200*.3030) )      )#, 
bd = bd +       rep( integer(0),        200*.6970  )   
bd
bd =       rep( 1,ceiling(200*.3030) )      
bd = bd +  rep( 0,        200*.6970  )   
bd

# interger won't work.  rep create a lists/vector, which is why is numeric

str(bd)
sim_01sn = data.frame(birth_id, bd)
str(sim_01sn)            
sim_01 = rbinom(n=200,size=1,prob=0.3030)  # think of the rbinom as "200 choose 1, with prob of 30.30%"
?rbinom()

## lecture did NOT cover rbinom!!

#sim_01 <- bd
sim_01
class(sim_01)
class(bd)
expect_true("integer" == class(sim_01))     # sim_01 via rbinom() is integer *sigh()
expect_true("integer" == class(bd))         # somehow bd is not integer!?!?
class(bd)
```
...
output_01 = sim_01 %>%  summarize( numBd = sum(birth_defect), proportionBd = mean(birth_defect))
# 71, .355
# 56, .28
...
sim_02 = rbinom(n=200,size=1,prob=0.3030) 
sim_03 = rbinom(n=200,size=1,prob=0.3030)  
sim_04 = rbinom(n=200,size=1,prob=0.3030)  
sim_05 = rbinom(n=200,size=1,prob=0.3030)  
str(sim_02)
output_02 = as.data.frame(sim_02) %>% rename(birth_defect = sim_02) %>%  summarize( numBd = sum(birth_defect), proportionBd = mean(birth_defect))
output_03 = as.data.frame(sim_03) %>% rename(birth_defect = sim_03) %>%  summarize( numBd = sum(birth_defect), proportionBd = mean(birth_defect))
output_04 = as.data.frame(sim_04) %>% rename(birth_defect = sim_04) %>%  summarize( numBd = sum(birth_defect), proportionBd = mean(birth_defect))
output_05 = as.data.frame(sim_05) %>% rename(birth_defect = sim_05) %>%  summarize( numBd = sum(birth_defect), proportionBd = mean(birth_defect))
...
#p5 = c(output_01, output_02, output_03, output_04, output_05)
p5 = sort( c(output_01$proportionBd, output_02$proportionBd, output_03$proportionBd, output_04$proportionBd, output_05$proportionBd) )
#p5 = sort(tmp5)
#p5 = c(sim_02, sim_02, sim_03, sim_04, sim_05)
...
_somewhat close, but fluctuated more than i expected, making go back to see if the code was buggy._
...
_I expect the simulation with 1000 elements to produce closer to the 30.30% proportion, with fewer variance.  size matter, especially sample size!_
...
# Then calculate the probability that a randomly chosen American will have HCV. 
# Convert to percentages and round to two decimal places. Save the HIV and HCV probabilities (without the %) to the vector called `p8`.**

#rbinom(n=321000000,size=1, prob=1100000/321000000)

prob_hiv=round(1100000/321000000*100,2)
prob_hcv=round(3500000/321000000*100,2)
p8 = c(prob_hiv, prob_hcv )
p8
```
...
oh, can use this fact:
  Among individuals with HIV, approximately 25% have Hepatitis C.

so, P(HCV|HIV)=25%

Bayes theorem provides
P(HCV|HIV) = P(HIV|HCV) * P(HIV) / P(HCV)
rearrange:
P(HCV|HIV) / P(HIV|HCV) = P(HCV)/P(HIV)
nah, don't need Bayes.  just def of cond prob.

P(HCV|HIV)=P(HCV & HIV)/ P(HIV)
thus
P(HCV & HIV) = P(HCV|HIV) * P(HIV)
             = 0.25       * frac_hiv  = ... 0.09%
...
frac_hiv = 1100000/321000000
frac_hcv = 3500000/321000000
tmp9 = frac_hiv * frac_hcv
tmp9 * 100
p9 = round( tmp9 * 100, 2 )
p9

frac_hcv_given_hiv = 0.25

p9 = round(0.25 * frac_hiv * 100, 2)    # 0.09%, answer from solution pdf
p9
...
_if independent, then Prob(HIV intersect HCV) is empty. but research has found that it is not, which is why was given the value 300,000_
...
if they were independent, then would be the same.  but they are not independent.
...
```{r}
(frac_hcv * frac_hiv) / frac_hiv
frac_hcv

# p9 is P(HIV intersect HCV) 

frac_hcv_given_hiv = 0.25
frac_hcv_and_hiv = frac_hcv_given_hiv * frac_hiv
frac_hcv_and_hiv # 0.000856

frac_hiv_given_hcv = frac_hcv_and_hiv / frac_hcv
frac_hiv_given_hcv # 0.078    which differ from frac_hcv_given_hiv (0.25)  thus they are not generally the same

## head is spinning, need to write math in paper!
...
//++sn50++ stopped here 2022.0228 1140pm
...
_C: Lung Cancer in a given year_
_S: Heavy Smoker_
_P(C|S)=3%     ... Hmm, solution only use P(C)=0.03_
_Sensitivity: P(T+|C+)=89%  This indicate those who have Lung Cancer and correctly test +ve _
_Specifity:   P(T-|C-)=93%  Those that don't have cancer and correctly get -ve test result._
...
what's target pop?  Heavy smoker?  or adults older than X years?
...
P(T+) = 94.6/100 = 0.946 ie 9.46% will test +ve in this pop.


study = medicare enrolled heavy smoker
...
Actually, looking at asnwer in Q15, seems should have done T as first leve, then C as second level.  thus:
...
P(C+|T+) = 26.7/94.6 = 0.2822

# this is def that fit PPV... EPI said to relate to FEASIBILITY, not a very high number, which is why no CT scan of lung?
...
p15 = round(26.7/94.6 *100, 1) # 0.2822
#p15 = 28.2
```
