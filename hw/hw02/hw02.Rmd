...
###
Mean: calculated by taking all values of x, sum them, then divide by number of elements.
Median: split the data in exactly half, find this 50th percentile, and read its x value.
###
...
#?median
?summarize
GDP_summary <- CS_data %>% summarize(mean_GDP = mean(GDP_2006), median_GDP = median(GDP_2006))
GDP_summary
...
#### duh, that's why i failed grader!!
...
###        geom_vline(aes(xintercept = mean(GDP_2006)  ),  lty=1) + 
...
as income increases, ceasrean rate increases

\newpage

**10. [1 point] Why might lower income countries have lower rates of cesarean delivery?**

C-sesction is not a cheap procedure, thus not affordable to the masses in lower income country?   Or sanitation is not as good in lower income country, rendering c-section more risky than high income country, thus less favorable?
...
CS_summary =  CS_data %>% summarize(mean_CS = mean(CS_rate_100), median_CS = median(CS_rate_100))
...
## boxplot, lecture 04 material

p12 = ggplot(CS_data, aes(y=CS_rate_100)) + geom_boxplot() 
p12
...
#five_num_summary = ( Min, Q1, Median, Q3, Max )
...
p14 <- ggplot(data = CS_data, aes(y = CS_rate_100)) +
          ...
          geom_hline(aes(yintercept = 23.4), col = 'blue') +
...
CS_data2 = CS_data_plus_2

Min = min(CS_data2$CS_rate_100)
Q1 = quantile(CS_data2$CS_rate_100, probs = 0.25)
Median = median(CS_data2$CS_rate_100)
Q3 = quantile(CS_data2$CS_rate_100, probs = 0.75)
Max = max(CS_data2$CS_rate_100)

five_num_summary2 = CS_data2 %>% summarize( min = Min, 
                                          Q1 = Q1,
                                          median = Median,
                                          Q3 = Q3, 
                                          max = Max)
...
five_num_summary2
fns2 = five_num_summary2

p14b = ggplot(data = CS_data2, aes(y = CS_rate_100)) +
          geom_boxplot(col = 'black', fill = 'sienna2') +
          theme_minimal() + 
          geom_hline(aes(yintercept = 0.4), col = 'blue') +
          geom_hline(aes(yintercept = 5.1), col = 'blue') +
...
###
It added two numbers in the far "high" end, creating a shift/skew.
outliers!
