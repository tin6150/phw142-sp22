...
Exam 1 review concept slide 21

```{r}

OFC1=c(.379,4.1,2.01,12.6,4.98,1.92)
OFC2=c(6.31,7.94, 9.46,11.0, 11.1, 8.9)
##XXOFCdata = as.array(matrix( data=c(OFC1, OFC2), ncol = 2))
OFCdata = data.frame(OFC1,OFC2)

#### matrix isn't a dataframe :-\
...
rev1 = ggplot(data=OFCdata, aes(x=log(OFC1), y=OFC2)) + geom_point()
rev1
```
...
#!pwd
##q9df = read.csv("/mnt/c/Users/tin61/tin-gh/inet-dev-class/r_stat142/exam/simpleDF.csv")
q9df = read.csv("c:/Users/tin61/tin-gh/inet-dev-class/r_stat142/exam/simpleDF.csv")
#system("uptime")
czs_tin = q9df %>% filter(status=="True") %>% group_by(zip) %>% tally()
czs_tin
q9df  
  # summarize( count_zip_status = count() )

##q9df %>% group_by(zip) %>% summarize( count_zip_status = filter(status=="True"))


## ++ where is the count thing... there was a simpler way in R ++ ...

lung_data = q9df %>% rename(zip_code = zip)

#solution answer:
count_zip_status <- lung_data %>% group_by(zip_code) %>% summarize(count = sum(boolero))
count_zip_status

czs = lung_data %>% count()
czs = lung_data %>% group_by(zip_code) %>% count()
czs
czs2 = lung_data %>% group_by(zip_code) %>% summarize( nnn = count() )

count_zip_status <- lung_data %>% group_by(zip_code) %>% summarize(cuenta = tally(status==TRUE))
...
# next one error, True is not found, if text, need quote.
bazz=True
bazz
